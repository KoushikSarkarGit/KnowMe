...
export function AboutMeGlowingCards({ payload }: GlowingCardsProps) {
  return (
    <div
      className={`relative   w-full md:h-[18rem] rounded-2xl  p-2 md:rounded-2xl md:p-2`}
    >
      <GlowingEffect
        spread={40}
        glow={true}
        ...
        inactiveZone={0.01}
      />
      <div className="border-0.75 relative flex h-full  flex-col justify-between dark:bg-black  overflow-hidden rounded-xl px-5 py-5   dark:shadow-[0px_0px_15px_0px_#A9A9A9]">
        <div className="flex flex-col  items-center justify-start gap-1">
          <div className="w-full  bg-white  rounded-lg  ">
            <img
              src={payload.image}
              className="object-cover rounded brightness-95"
              alt="logo"
            />
          </div>
